...
GOFLAGS=-v -ldflags '-X "$(REPO)/cmd.Version=$(version)" -X "$(REPO)/cmd.Commit=$(commit)" -X "$(REPO)/cmd.BuildDate=$(builddate)"'
...
debian_base:
	...
	cp -r $(BINDIR)/$(NAME)-$(arch) $(PACKAGE_DIR)/$(EXTRACT_DIR)/$(NAME)
	chmod +x $(PACKAGE_DIR)/$(EXTRACT_DIR)/$(NAME)
